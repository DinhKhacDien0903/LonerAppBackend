...
public class PromptHandler : IRequestHandler<PromptRequest, Result<PromptResponse>>
{
    ...
    private const string DEFAULT_MAX_LINE = "Hãy trả lời ngắn gọn trong tối đa 7 dòng";
    ...
    public async Task<Result<PromptResponse>> Handle(PromptRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var botMessage = new MessageEntity
            {
                ...
                SenderId = request.Request.ReceiverId ?? "", // TODO: Add if for chat bot in User table.
                Content = result,
                IsMessageOfChatBot = true,
                IsImage = false,
                CreatedAt = DateTime.UtcNow
            };
            var newMessage = InitMessage(request.Request);
            await _uow.MessageRepository.AddAsync(newMessage);
            await _uow.MessageRepository.AddAsync(botMessage);
            await _uow.CommitAsync();
            if (!string.IsNullOrEmpty(result))
            {
                return Result<PromptResponse>.Success(new PromptResponse(result));
            }
            else
            {
